...
Let  $x, y$ be the dimensions of  $X$ and  $Y$ respectively. We can express  $f$ as an  $y\times x$ matrix. For the contravariance of the functor we have that the morphism $Y^* \to X^*$ must be $A^T$. If we repeat this process we are given that $F(F(A))=(A^T)^T=A \ \forall A$.

(terminar)


\prob{3}{Show that two categories $\mathcal{B}$ and $\mathcal{C}$
}
