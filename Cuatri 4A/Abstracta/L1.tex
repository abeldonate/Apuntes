...
We start by noticing if we have a finite dimensional vector space we can think as morphisms as matrices. Let $X^*=Hom(X,\mathbb{R})$ and $Y^*=Hom(Y, \mathbb{R})$. Let $F$ be the contravariant functor that dualizes. We have the following diagrams:

% https://q.uiver.app/#q=WzAsNixbMCwwLCJYIl0sWzAsMSwiWSJdLFsxLDAsIlheKiJdLFsyLDAsIlheeyoqfVxcY29uZyBYIl0sWzIsMSwiWV57Kip9XFxjb25nIFkiXSxbMSwxLCJZXioiXSxbMCwyLCJGIl0sWzIsMywiRiJdLFs1LDQsIkYiXSxbMSw1LCJGIl0sWzAsMSwiQSIsMl0sWzUsMiwiQV5UIl0sWzMsNCwiQT1BXntUVH0iLDJdXQ==
\[\begin{tikzcd}
	X & {X^*} & {X^{**}\cong X} \\
	Y & {Y^*} & {Y^{**}\cong Y}
	...
	\arrow["{A^T}", from=2-2, to=1-2]
	\arrow["{A=A^{TT}}"', from=1-3, to=2-3]
\end{tikzcd}\]


Let  $x, y$ be the dimensions of  $X$ and  $Y$ respectively. We can express  $f$ as an  $y\times x$ matrix. For the contravariance of the functor we have that the morphism $Y^* \to X^*$ must be $A^T$. If we repeat this process we are given that $F(F(A))=(A^T)^T=A \ \forall A$, and we get the natural transformation.
