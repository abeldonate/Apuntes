...
$A^*$ es una $\sigma-$álgebra que contiene $A$, y por tanto  $(X, \mathcal{A}^*)$ es un espacio mesurable. Si $E_n$ es disjunta de  $\mathcal{A}^*$:
\[
...
Queremos fabricar una mesura $l$ en $\mathbb{R}$ que coincida con las longitudes de los intervalos
\begin{proposition}
La colección $\mathcal{F}$ de uniones finitas de los conjuntos $\emptyset, (a, b], (-\infty, b], (a, +\infty), (-\infty, +\infty)$ es un álgebra de conjuntos de $\mathbb{R}$ con mesura $l$
\end{proposition}

Se cumple $\mathcal{F}\subset \mathcal{B} \subset \mathcal{F}^* \subset \mathcal{P}(\mathbb{R})$
...
\begin{theorem}
 El espacio $(L_p, \|\cdot \|_{p}), p\ge 1$ es normado y completo $\implies$ es un espacio de Banach.
\end{theorem}
