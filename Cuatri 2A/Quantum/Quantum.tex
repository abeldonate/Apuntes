...
\end{itemize}
$$
mvr = pr = n\hbar, \qquad p = \frac{h}{\lambda} \implies 2\pi \boxed{r=n\lambda}
$$
...
$$
R_\infty = \left( \frac{1}{4\pi \epsiln_0}\right)^2\frac{me^4}{4\pi \hbar^3 c}, \qquad R_M = \frac{\mu}{m}R_\infty 
$$
...
If we introduce a magnetic field, the particle with magnetic moment $u_l$ experiences a torque $\tau = u_l \times B$ and change of potential energy  $\Delta E = -u_l \cdot B$ \\
\\
If the field is not uniform:
\[
F_z = \frac{dB_z}{dz} u_l \hat{z}
\] 
The spin is a quantum number that does not come from the resolution of Schrödinger equation, but is added to match the theoretical developement with the experiments. \\
...
\\
\textbf{Strong Exclusion Principle:} A system containing several electrons must be described by an antisymmetric eigenfunction \\
